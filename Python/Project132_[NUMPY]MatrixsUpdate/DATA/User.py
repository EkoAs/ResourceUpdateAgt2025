...
def matrix_Math():
    ...
    match operator:
        case "1": 
            while(True):
                Matrixs.matrix_pangkat_costum()
                if not input("Apakah Anda ingin melanjutkan Rumus Matriks? (y/n): ").lower() != 'y':
                    break
        case "2": 
            while(True):
                Matrixs.matrix_operator()
                if not input("Apakah Anda ingin melanjutkan Rumus Matriks? (y/n): ").lower() != 'y':
                    break
        case "3": 
            while(True):
                MatrixInvers.matrix_invers()
                if not input("Apakah Anda ingin melanjutkan Rumus Matriks? (y/n): ").lower() != 'y':
                    break
        case "4": 
            while(True):
                MatrixsTranspose.matrix_transpose()
                if not input("Apakah Anda ingin melanjutkan Rumus Matriks? (y/n): ").lower() != 'y':
                    break
...
def barisdanderet_Opsi():
    ...
    match operator:
        ...
        case "2": BarisDeret.barisan_Geometri()
        case "3": BarisDeret.deret_tak_hingga()
        case "4": BarisDeret.deret_fibonacci()
        case _: print(f"Opsi tidak sesuai!.")