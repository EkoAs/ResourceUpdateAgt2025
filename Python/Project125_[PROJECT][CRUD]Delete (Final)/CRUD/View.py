...
def delete_console():
    ...
    while(True):
        ...
        if data_buku:
            ...
            print("\n","="*100)
            print("Silakan pilih data apa yg ingin di hapus :")
            print(f"1. Judul ({judul:.40})")
            print(f"2. Penulis ({penulis:.40})")
            print(f"3. Tahun ({str(tahun):.40})")
            done = input("apakah yakin akan di hapus? (y/n): ") # lanjut atau selesai. kaloo lanjut kembali ke menu awal
            if done.lower() != 'n':
...
def update_console():
    ...
    while(True):
        ...
        if done.lower() != 'n':
            print(f"{'Data yang sudah di Update':-^100}")
            read_console()
            break
...
def read_console():
    ...
    # Header
    print("\n"+"="*100)
    print(f"{noIndex:4} | {judul:^40} | {penulis:^40} | {tahun:4}")
    ...
    for index,data in enumerate(data_file):
        ...
        if len(data_break) < 5:
            print(f" Baris rusak di nomor {index+1}, data: {data_break}")
